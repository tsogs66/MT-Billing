...
export default function Topbar({ title }: { title: string }) {
  ...
  return (
    <header className="theme-topbar sticky top-0 z-30 min-h-16 h-16 flex items-center justify-between gap-1.5 sm:gap-2 px-2.5 sm:px-6 lg:px-8 pt-[env(safe-area-inset-top)]">
      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
        <button
          type="button"
          onClick={toggleSidebar}
          className="theme-topbar-icon-btn lg:hidden p-2 shrink-0"
          aria-label="Open menu"
        >
          <Menu size={20} />
        </button>
        <div className="min-w-0 hidden min-[380px]:block">
          <h1 className="theme-topbar-title text-base sm:text-xl font-bold tracking-tight truncate max-w-[38vw] sm:max-w-none">{title}</h1>
      ...
      <div className="flex items-center gap-1 sm:gap-3 min-w-0 shrink-0">
        <div className="relative" ref={routerRef}>
          <button
            ...
            className="theme-topbar-pill text-sm max-w-[5.75rem] sm:max-w-none"
          >
            ...
            <span className="theme-topbar-pill-name sm:hidden max-w-[3.75rem] truncate">{current?.name ?? 'None'}</span>
            <ChevronDown
              size={14}
          ...
          {routerOpen && (
            <div className="theme-topbar-menu absolute right-0 mt-2 w-[min(16rem,calc(100vw-1rem))] py-2 z-[600] animate-scale-in origin-top-right">
        ...
        <div className="relative" ref={themeRef}>
          <button
            type="button"
            className="theme-topbar-icon-btn p-2 shrink-0"
            title="Theme"
          ...
          {themeOpen && (
            <div className="theme-topbar-menu absolute right-0 mt-2 w-[min(14rem,calc(100vw-1rem))] py-2 z-[600] animate-scale-in origin-top-right">
              <div className="theme-topbar-menu-muted px-3 py-2 text-[10px] font-bold uppercase tracking-wider">
                Theme
              </div>
        ...
        <div className="relative" ref={userRef}>
          <button
            ...
            className="theme-topbar-user-btn shrink-0"
          >
          ...
          {userOpen && (
            <div className="theme-topbar-menu absolute right-0 mt-2 w-[min(13rem,calc(100vw-1rem))] py-2 z-[600] animate-scale-in origin-top-right">
